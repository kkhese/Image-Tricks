...
unsigned char Shading(float P[3], float N[3], float kd)
{
    ...
    noisez = perlinNoise(P[0], P[1], P[2], 0);
    noisex = perlinNoise(P[0], P[1], P[2], 1);
    noisey = perlinNoise(P[0], P[1], P[2], 2);
    ...
    // Add Perlin noise to Normal vector
    N[0] = N[0] * (1.0f + 0.0 * noisex);
    N[1] = N[1] * (1.0f + 0.0 * noisey);
    N[2] = N[2] * (1.0f + 0.0 * noisez);
    // Normalize N
    N[0] = N[0] / Length(N);
    N[1] = N[1] / Length(N);
    N[2] = N[2] / Length(N);
    ...
    if (Ctemp > 1.0) {
        // Add Perlin noise to shading value C
        C = (unsigned char)(unsigned int)(Ip * kd * (N[0] * L[0] + N[1] * L[1] + N[2] * L[2]));
        C = (unsigned char)(unsigned int) (C * (1.0 + sqrt(noisex * noisex + noisey * noisey + noisez * noisez)/1.0));
    }
	// if angle bet. light and normal is greater than or equal with 90.
	// force total black.
    else {
        // Add Perlin noise to Dark side for making ice cream ball
        C = (unsigned char) 1;
        C = (unsigned char)(unsigned int)(C * (1.0 + sqrt(noisex * noisex + noisey * noisey + noisez * noisez) / 0.01));
    }
    return (C);
}
...
float RSintersection(float P0[3], float V[3], float N[3]) {
    ...
    // we only need near one between t0 and t1, no need tca + thc
	float t = tca - thc;

    float noisez, noisex, noisey;
    //call Perlin Noise
    noisez = perlinNoise(P0[0] + t * V[0], P0[1] + t * V[1], P0[2] + t * V[2], 0);
    noisex = perlinNoise(P0[0] + t * V[0], P0[1] + t * V[1], P0[2] + t * V[2], 1);
    noisey = perlinNoise(P0[0] + t * V[0], P0[1] + t * V[1], P0[2] + t * V[2], 2);

    // Add Perlin noise to radius: rr is radius^2
    rr = ((P0[0] + t * V[0]) * (1.0 + 0.45 * noisex) - obj1.x) * ((P0[0] + t * V[0]) * (1.0 + 0.45 * noisex) - obj1.x) +
        ((P0[1] + t * V[1]) * (1.0 + 0.45 * noisey) - obj1.y) * ((P0[1] + t * V[1]) * (1.0 + 0.45 * noisey) - obj1.y) +
        ((P0[2] + t * V[2]) * (1.0 + 0.45 * noisez) - obj1.z) * ((P0[2] + t * V[2]) * (1.0 + 0.45 * noisez) - obj1.z);

    // Re-calculate thc since rr changed
    thc = sqrt(rr - dd);
    // If r is lager than or equal with d, it meet
	if ( dd <= rr) {
		// re-calculate t since thc changed
        t = tca - thc;
       
        // Add Perlin noise to t
        t = t * (1.0+  0.7* sqrt( noisex*noisex + noisey*noisey + noisez*noisez));

		// Normal Vector is from origin of sphere to intersection point
        // Add additional Perlin noise to N
        N[0] = ((P0[0] + t * V[0]) * (1.0 + 0.55*noisex) - obj1.x);
        N[1] = ((P0[1] + t * V[1]) * (1.0 + 0.55*noisey) - obj1.y);
        N[2] = ((P0[2] + t * V[2]) * (1.0 + 0.55*noisez) - obj1.z);
...
float RPintersection(float P0[3], float V[3], float N[3]) 
{
    ...
    noisey = perlinNoise(P0[0] + t * V[0], P0[1] + t * V[1], P0[2] + t * V[2], 2);
 	// Intersection point
    // Add Perlin noise to intersection point
    float rayPoint[3] = { (P0[0] + t * V[0])*(1.0+0.5*noisex), (P0[1] + t * V[1])*(1.0+0.0*noisey), (P0[2] + t * V[2])*(1.0+0.5*noisez) };
